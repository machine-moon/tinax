...
`tinax.random` owns typed scalar key validation, ordered deterministic coordinate derivation, and explicit continuation-key ownership. It never discovers process identity or stores mutable RNG state.
...
::: tinax.random.derive_key

::: tinax.random.derive_process_step_key

::: tinax.random.split_key
