"""Shared validation for concrete layouts, shard_map, and process-local array construction."""
...
def has_cross_process_replicas(sharding: _jax.sharding.NamedSharding, shape: tuple[int, ...]) -> bool:
...
def require_manual_axes(mesh: _jax.sharding.Mesh, axis_names: frozenset[str]) -> frozenset[str]:
    unknown = axis_names - set(mesh.axis_names)
    if unknown:
        raise ValueError(f"axis_names must be a subset of the mesh's axes, unknown: {sorted(unknown)}")
    effective = axis_names if axis_names else frozenset(mesh.axis_names)
    axis_types = dict(zip(mesh.axis_names, mesh.axis_types, strict=True))
    non_manual = tuple(name for name in effective if axis_types[name] is not _jax.sharding.AxisType.Manual)
    if non_manual:
        raise ValueError(f"shard_map requires Manual mesh axes, got non-Manual axes: {sorted(non_manual)}")
    return effective


def _spec_axis_names(spec: _jax.sharding.PartitionSpec) -> tuple[str, ...]:
    names: list[str] = []
    for entry in spec:
        if entry is None:
            continue
...
def validate_specs_against_axes(specs: object, allowed_axes: frozenset[str], name: str) -> None:
    leaves = _jax.tree_util.tree_leaves(specs, is_leaf=lambda leaf: isinstance(leaf, _jax.sharding.PartitionSpec))
    for leaf in leaves:
        if not isinstance(leaf, _jax.sharding.PartitionSpec):
            raise TypeError(f"{name} leaves must be jax.sharding.PartitionSpec instances")
        unknown = set(_spec_axis_names(leaf)) - allowed_axes
        if unknown:
            raise ValueError(f"{name} references axes outside the effective manual set: {sorted(unknown)}")
