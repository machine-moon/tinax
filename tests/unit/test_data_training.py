...
from tinax.data import training_batches
...
def test_training_processes_are_exactly_disjoint_with_equal_steps() -> None:
    process_batches = [
        list(_training(size=17, index=index, count=3, seed=5, shuffle=True, num_epochs=3)) for index in range(3)
    ]
    expected = list(grain.MapDataset.range(17).seed(5).shuffle().slice(slice(0, 12)).repeat(3, reseed_each_epoch=True))
...
def test_training_batches_never_cross_finite_epoch_boundaries() -> None:
    process_batches = [list(_training(size=11, index=index, count=2, num_epochs=2)) for index in range(2)]
