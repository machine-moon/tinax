...
import tinax.sharding as sharding_api
...
def test_host_copy_snapshots_before_asynchronous_placement(
    explicit_mesh: Mesh, axis_name: str
) -> None:
...
def test_process_local_data_validates_addressable_extents_and_policy(
    explicit_mesh: Mesh, axis_name: str
) -> None:
    rows = explicit_mesh.shape[axis_name] * 2
...
def test_process_local_copy_allows_immediate_source_mutation(
    explicit_mesh: Mesh, axis_name: str
) -> None:
    rows = explicit_mesh.shape[axis_name] * 2
    source = np.arange(rows * 3, dtype=np.int32).reshape(rows, 3)
...
def test_process_local_data_supports_scalars_and_empty_dimensions(
    explicit_mesh: Mesh, axis_name: str
) -> None:
