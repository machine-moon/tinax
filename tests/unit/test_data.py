...
from tinax.data._generic import BatchTransform
...
def test_package_exports_stable_data_primitives() -> None:
    assert data_api.InMemorySource is InMemorySource
    assert data_api.training_batches is training_batches
    assert data_api.evaluation_batches is evaluation_batches
    assert data_api.read_array_record is read_array_record
    assert data_api.write_array_record is write_array_record
    assert data_api.read_parquet_record is read_parquet_record
    assert data_api.write_parquet_record is write_parquet_record
    assert data_api.split_by_column is split_by_column
    assert data_api.split_random is split_random
    assert callable(data_api.open_multiprocessing_iterator)
    assert not hasattr(data_api, "snapshot_iterator_state")
    assert not hasattr(data_api, "restore_iterator_state")
